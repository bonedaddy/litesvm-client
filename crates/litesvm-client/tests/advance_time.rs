...
fn test_advance_time() {
    ...
    let current_time: i64 = AsRef::<LiteSVM>::as_ref(&svm_client).get_sysvar::<Clock>().unix_timestamp;

    svm_client.advance_time(current_time+1);

    let new_time: i64 = AsRef::<LiteSVM>::as_ref(&svm_client).get_sysvar::<Clock>().unix_timestamp;
